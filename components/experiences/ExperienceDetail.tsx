...
export default function ExperienceDetail({
  ...
  const hasGallery = experience.gallery && experience.gallery.length > 0;
  ...
  return (
    <>
            ...
            <div className="pt-6 border-t border-white/10 mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {experience.skills.map((skill) => (
                  <span key={skill} className="px-2.5 py-1 rounded bg-white/5 border border-white/5 text-xs text-white/60 font-mono">
                    {skill}
                  </span>
                ))}
              </div>
      ...
      <div className="hidden md:grid md:grid-cols-[1.5fr_1fr] h-full bg-card/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">

        {/* LEFT COLUMN: Narrative & Details */}
        <div className="flex flex-col h-full overflow-y-auto p-10 scrollbar-hide relative">
        ...
        <div className="h-full bg-black/40 relative overflow-hidden border-l border-white/10">
          {hasGallery ? (
            <DesktopGallery images={experience.gallery!} />
          ) : (
